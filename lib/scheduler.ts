...
export function applyReview(current:Progress|undefined,cardId:string,direction:Direction,known:boolean):Progress{
 ...
 const interval=known?INTERVALS[Math.min(Math.max(successCount-1,0),INTERVALS.length-1)]:0;
 return {...item,cardId,direction,successCount,failureCount,learned,dueAt:addDays(interval),lastReviewedAt:new Date().toISOString()};
}
